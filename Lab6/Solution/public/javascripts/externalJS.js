...
function populateContactList() {
    ...
    // jQuery AJAX call for contactList
...
function updateContact(existingContact) {
    ...
    $.ajax({
        ...
        if (response.msg === '') {
			...
			if ($('#contactInfoName').val() == existingContact.name) {
    ...
    if (confirmation === true) {
       ...
       var id = $(this).attr('rel');
        $.ajax({
            ...
            if(response.msg === ''){
				//delete contact info display if the deleted 
				//contact's information is being displayed
